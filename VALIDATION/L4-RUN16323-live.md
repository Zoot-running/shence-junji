...
## 收官与终稿（07:00）
- **最终成绩：18800/20800（90.38 分），37/40 flags**，elapsed 21313s（我手动 finish 停表）。
- 丢分原因：**WSL 发行版 06:56 第三次整体重启**（dmesg 实锤，前两次 9/8 10:26:26/10:26:47），
  战役三件套（guard/agent/watcher/jintuo）随发行版消亡且无人拉起（Windows 侧 daemon 只保生产 web）——
  04:17 后战役停摆 2.5 小时，最后 3 hard（g-20/27/28，2000 分）未来得及打。
- **F24（新架构缺口）**：金柝只守进程（发行版会话内），发行版重启后整栈无自动复活机制。
  改进：Windows 侧或 WSL boot 钩子（/etc/wsl.conf [boot] command 或登录脚本）拉起
  jintuo+guard+watch 三件套——"守护的守护"。
