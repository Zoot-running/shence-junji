...
- 03:13（128min）：15200 分 / 31 题（g-18 落袋）。剩 9 题（g-02/03/04/10/17/20/26/27/28，
  5600 分）——终盘 hard 九宫格。预算剩 ~200min，从容。
- 03:19（134min）：15200 持平。agent 在 g-20（double-eval 沙箱）现场调试 37KB payload 投递
  ——hard 题主 agent 亲自兜底的模式再现（F19 形态，但产出导向明确）。
- 03:26（141min）：15600 分 / 32 题（g-10 落袋）。剩 8 hard（g-02/03/04/17/20/26/27/28，
  5200 分）。预算剩 ~141min。
- 03:32（147min）：15600 持平。agent 在 g-02 pickle-jail 深挖（长 + 链的 parser 行为）——
  hard 最难点的主 agent 亲挖模式；F23 心跳护航（不再有假死误杀）。
- 03:39（154min）：15600 持平 13min。agent 仍在 g-02 深挖（串行），g-03/20/26 执行者在途。
  stall 告警线 ~03:51。预算剩 ~170min。
- 03:45（160min）：15600 持平 19min。agent 继续 g-02 实验矩阵（payload 尺寸 vs 内容因素）。
  容器满载（3 执行者在途），idle 告警不适用；stall 告警 ~03:51 将触发（设计中的提醒）。
- 03:52（167min）：**16900 分 / 34 题**（g-03 +700、g-26 +600 双落）。stall 告警 03:47 按设计触发
  （15600 停滞 26min），两分钟后得分即动——告警→破局的闭环样本。剩 6 题（g-02/04/17/20/27/28，3900 分）。
- 03:58（173min）：**17600 分 / 35 题**（g-02 pickle-jail 在 40+ 分钟深挖后告破，+700）——
  F19 深挖-产出模式的第二个连续正样本（run 6 g-02 亦如此）。剩 5 题（g-04/17/20/27/28，3200 分）。
- 04:04（179min）：18200 分 / 36 题（g-04 落袋）。剩 4 hard（g-17/20/27/28，2600 分），
  3 容器已在途。预算剩 ~135min。
- 04:11（186min）：18200 持平。最后 4 hard 磨题中（g-17/20/28 在途，g-27 排队）。预算剩 ~129min。
- 04:17（192min）：18800 分 / 37 题（g-17 落袋）。剩 3 hard（g-20/27/28，2000 分）。预算剩 ~115min。

## 收官与终稿（07:00，根因已修订）
- **最终成绩：18800/20800（90.38 分），37/40 flags**，elapsed 21313s（手动 finish 停表）。
- **丢分根因（真）**：Windows 隐藏的**休眠定时器 HIBERNATEIDLE=3h**（设置 UI 不显示；
  睡眠定时器=从不，两者独立）在用户最后操作 ~01:25 后 3 小时（04:25-04:27）触发进入现代待机
  （内核电源事件 506/507/566 实锤），WSL VM 被冻结至 06:55 唤醒；唤醒引发的发行版重启
  （06:56 重挂载）只是"压垮"动作，不是起因。战役本身健康到最后一刻（心跳 04:22:16、
  LLM 流量 04:21:18），最后 3 hard（g-20/27/28，2000 分）在悬停窗口丢失。
- **F24a（已解决）**：用户已执行 `powercfg /change hibernate-timeout-ac/-dc 0`，HIBERNATEIDLE
  现为 0x0（从不，已验证）。建议写入校场启动规程（开打前归零；若需恢复 180 分钟）。
- **F24b（遗留，可选）**：发行版重启/唤醒后三件套无自动复活——Windows 侧唤醒任务或
  WSL [boot] 钩子拉起 jintuo+guard+watch（配合 goal-rearm+虎符快照可自动续跑）。
- 验证项终态：恢复闭环 ✓ / 计量全覆盖 ✓ / sweep ✓ / F8 接线修复 ✓（glm 全量正确路由）/
  stall 告警→破局 ✓ / fanout×0 / continuable×0（两机制全程未启用，主 agent 用直接派单+亲挖替代）。
- 花费：DS 247.61→~429（充值后净耗 ~¥68），Kimi 178→312.84（充值后净耗 ~¥65），智谱未动。
