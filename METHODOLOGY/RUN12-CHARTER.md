...
- **结果**：验收 6 项全过——终局 17583 双题入账 600 分、elapsed 211s、赛后会话/
  用量/事件可拉、hosted-replay 重建成功。过程六次迭代（17553→17583），每次故障
  均从平台会话原文定位当天修复。
- **决策-结果**：D2 打包审计抓到 dev-daemon.sh 密钥与 `.agents` 私知两枚**真雷**
  （若进镜像直接构成规则 6 作弊）——打包管线价值实证；D4 不猜平台行为 → 零误判。
- **根因**：三处镜像配置缺失（effort 声明/沙箱后端/DSH_PERMISSION_MODE env），
  无平台侧故障。
- **教训分类**：A="托管镜像=本地完整复刻,差异用平台会话日志闭环"升格为打包
  checklist；B=机制发现 6 条入托管档案；D=无。
- **详见**：`VALIDATION/L4-RUN17553-17583-hosted-trial.md`。
